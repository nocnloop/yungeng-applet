    ...
    <view class="form-box">
      <u--form ref="form" labelPosition="left" :labelWidth="80" :rules="rules" :model="formData">
        <u-form-item label="车牌号" prop="carId" borderBottom @click="onSelectPlateNumber">
          <u--input :value="formData.carId" placeholder="请输入车牌号" border="none" disabled disabledColor="#ffffff"></u--input>
        </u-form-item>
        <u-form-item label="载重量" prop="loads" borderBottom @click="onSelectLoadNumber">
          <u--input :value="loadNumberFormater" placeholder="请选择载重量" border="none" disabled disabledColor="#ffffff"></u--input>
        </u-form-item>
        <u-form-item label="自重(T)" prop="weight" borderBottom>
          <u--input v-model="formData.weight" placeholder="请输入自重" type="number" border="none"></u--input>
        </u-form-item>
        <u-form-item label="车型" prop="carModel" borderBottom @click="onSelectCarModel">
          <u--input :value="carModelFormater" placeholder="请选择车型" border="none" disabled disabledColor="#ffffff"></u--input>
        </u-form-item>
...
import CardLoadNumberConfig from "@/config/car-load-number.js"
...
export default {
  data() {
    return {
      ...
      carLoadNumberColumns: [CardLoadNumberConfig],

      CardModelColumns: [CardModelConfig],
  ...
  computed: {
    loadNumberFormater() {
      const target = CardLoadNumberConfig.find(item => item.dictValue === this.formData.loads)
      return target && target.dictLabel
    },

    carModelFormater() {
      const target = CardModelConfig.find(item => item.dictValue === this.formData.carModel)
      return target && target.dictLabel
    }
  },

  onLoad(option) {
    this.showDefaultCar = option.canDefaultCar == "ok"
    this.formData.ifDefault = this.showDefaultCar ? 0 : 1
  },
